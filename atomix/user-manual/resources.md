...
Overloaded methods with `ConsistencyLevel` parameters are provided throughout Atomix's resources wherever it makes sense. In many cases, resources dictate the strongest consistency levels - e.g. [coordination](/atomix/user-manual/coordination) - and so weaker consistency levels are not allowed.
