...
```java
DistributedMap<String, String> map = atomix.create("/test-map", DistributedMap.class);
```

Atomix uses the provided `Class` to create an associated [StateMachine][state-machines] on each replica. This allows users to create and integrate [custom resources](#custom-resources).
...
Atomix clients and replicas communicate with each other through [sessions]. Each session represents a persistent connection between a single client and a complete Atomix cluster. Sessions allow Atomix to associate resource state changes with clients, and this information can often be used to manage state changes in terms of sessions as well.

Some Atomix resources expose a configurable `PersistenceMode` for resource state change operations. The persistence mode specifies whether a state change is associated directly with the client's `Session`. Atomix exposes two persistence modes:
...
Overloaded methods with `ConsistencyLevel` parameters are provided throughout Atomix's resources wherever it makes sense. In many cases, resources dictate the strongest consistency levels - e.g. [coordination](#distributed-coordination) - and so weaker consistency levels are not allowed.
...
set.add("Hello world!", PersistenceMode.EPHEMERAL).thenRun(() -> {
  // Close the Atomix instance to force the value to be removed from the set
  atomix.close();
});
```
...
map.put("foo", "Hello world!", PersistenceLevel.EPHEMERAL).thenRun(() -> {
  // Close the Atomix instance to force the key to be remove from the map
  atomix.close();
});
```
...
value.set("Hello world!", PersistenceMode.EPHEMERAL).thenRun(() -> {
  // Close the Atomix instance to force the value to be unset
  atomix.close();
});
```
...
To create a `DistributedTopic`, pass the class to `Atomix.create(String, Class)`:

```java
atomix.<DistributedTopic<String>>create("/test-topic", DistributedTopic.class).thenAccept(topic -> {
...
When a message is sent to a topic, the message will be logged and replicated like any state change via Atomix's underlying [Raft][raft-framework] implementation. Once the message is stored on a majority of servers, the message will be delivered to any client [sessions] alive at the time the message was sent.
